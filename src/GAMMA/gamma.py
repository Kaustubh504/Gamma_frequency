...
class GAMMA(object):
    def __init__(self,dimension, map_cstr=None, num_pe=64, pe_limit=1024, fitness="latency", constraints=dict(), par_RS=False, l1_size=512, l2_size=108000, NocBW=81920000, offchipBW=81920000, slevel_min=2,slevel_max=2, fixedCluster=0, log_level=2,constraint_class=None,external_mem_cstr=None, use_factor=False,uni_base=True, q_filter=None):
    ...
    def evaluate(self, pool, population, cur_gen=-1):
        ...
        # Only send genomes marked True to MAESTRO (via pool.map)
        # Genomes marked False get a synthetic -Inf result (no MAESTRO call)
        indices_to_eval = [i for i, m in enumerate(evaluate_mask) if m]
        pop_to_eval     = [population[i] for i in indices_to_eval]

        if pop_to_eval:
            partial_results = pool.map(self.thread_fun, pop_to_eval)
        else:
            partial_results = []

        # Reconstruct full reward_activ_list aligned with population indices
        reward_activ_list = [(None, None)] * len(population)
        for result_idx, pop_idx in enumerate(indices_to_eval):
            reward_activ_list[pop_idx] = partial_results[result_idx]

        n_evaluated = len(indices_to_eval)
        n_skipped   = len(population) - n_evaluated
        self._q_n_evaluated = n_evaluated   # read by run() for end_of_generation
        ...
        for i in range(len(population)):
            ...
            # ===== Track full genome record per individual =====
            indv = population[i]
            num_levels = len(indv) // 7
            record = {}

            # genome_str: full human-readable genome string
            # e.g. "L1:[Y,511] S=3,X=55,Y=4,R=3,K=33,C=34 || L2:[K,4] S=1,C=1,Y=1,X=1,R=1,K=1"
            level_strs = []
            for lvl in range(num_levels):
                seg = indv[lvl * 7: (lvl + 1) * 7]
                sp_dim, sp_sz = seg[0]
                record[f"L{lvl+1}_sp_dim"] = sp_dim
                record[f"L{lvl+1}_sp_sz"]  = sp_sz
                # store each gene WITH its position (loop order position 1-6)
                gene_parts = []
                for pos, gene in enumerate(seg[1:], start=1):
                    d, d_sz = gene
                    record[f"L{lvl+1}_pos{pos}_dim"]  = d     # which dimension
                    record[f"L{lvl+1}_pos{pos}_tile"] = d_sz  # tile size for that dim
                    gene_parts.append(f"{d}={d_sz}")
                level_strs.append(f"L{lvl+1}:[sp={sp_dim},{sp_sz}] " + ",".join(gene_parts))

            # compact full genome string — captures order + tile sizes
            record["genome_str"] = " || ".join(level_strs)
            record["fitness1"] = reward[0]
            record["fitness2"] = reward[1] if len(reward) > 1 else float("-Inf")
            record["valid"]    = int(reward[0] != float("-Inf"))
            # genome_key = full genome string (complete unique identifier)
            # genome_pattern = just sp-dim pattern like Y|K (for grouping only)
            genome_pattern = "|".join(indv[k][0] for k in range(0, len(indv), 7))
            record["genome_pattern"] = genome_pattern
            full_key = record["genome_str"]   # TRUE unique identifier
            fitness_val = reward[0]
            if full_key not in self.genome_seq_fitness or \
               (fitness_val != float("-Inf") and fitness_val > self.genome_seq_fitness[full_key]["fitness1"]):
                self.genome_seq_fitness[full_key] = record
            self.genome_all_records.append(record)
            # ====================================================
    ...
    def run(self, dimension, stage_idx=0, prev_stage_value=0, num_population=100, num_generations=100, elite_ratio=0.05,
        ...
        for g in range(num_generations):
            ...
            if self.q_filter is not None:
                # Count how many were actually evaluated vs skipped this gen
                # (stored during evaluate via evaluate_mask, recompute here)
                n_total = len(population)
                # Re-derive: skipped = those whose genome_all_records[-n_total:]
                # have valid==0 AND were not really called — approximate via
                # q_filter stats tracked inside evaluate_mask decisions.
                # Simplest: we exposed n_evaluated/n_skipped as instance vars.
                self.q_filter.end_of_generation(g, self._q_n_evaluated, self._q_n_skipped)
            # ─────────────────────────────────────────────────────────────────
            # self.check_tile_dependency(population)

            # ===== Genome type frequency counter (assignment requirement) =====
            self.genome_type_freq_history.append(
                self.print_genome_type_freq(population, gen=g + 1)
            )
            # ==================================================================
        ...
        pool.close()
        chkpt["genome_type_freq_history"] = self.genome_type_freq_history
        chkpt["genome_seq_fitness"] = self.genome_seq_fitness
        chkpt["genome_all_records"] = self.genome_all_records

        # ── Q-Filter: print summary and optionally save Q-table ──────────────
        if self.q_filter is not None:
            self.q_filter.print_summary()
            self.q_filter.save()   # saves only if q_table_path was set
        # ─────────────────────────────────────────────────────────────────────
    ...
    def count_genome_types(self, population):
        """
        Count the frequency of each spatial dimension type (K, C, Y, X, R, S)
        used as the parallelization dimension (sp) across all individuals
        and all cluster levels in the population.
        Returns a dict like {"K": 12, "C": 8, ...}
        """
        from collections import Counter
        freq = Counter()
        for indv in population:
            for k in range(0, len(indv), 7):
                sp_dim = indv[k][0]   # e.g. "K", "C", "Y", "X", "R", "S"
                freq[sp_dim] += 1
        # Ensure all dimension keys appear even if count is 0
        for dim in ["K", "C", "Y", "X", "R", "S"]:
            freq.setdefault(dim, 0)
        return dict(sorted(freq.items()))

    def print_genome_type_freq(self, population, gen=None):
        """
        Print the genome type frequency table for the current generation.
        """
        freq = self.count_genome_types(population)
        total = sum(freq.values())
        prefix = f"[Gen {gen}] " if gen is not None else ""
        print(f"{prefix}Genome type frequency (sp dimension across population):")
        for dim, cnt in freq.items():
            pct = cnt / total * 100 if total > 0 else 0
            print(f"  {dim}: {cnt:4d}  ({pct:.1f}%)")
        return freq
    ...
    def init_arguement(self, dimension=None, stage_idx=0, prev_stage_value=0, num_population=100, num_generations=100,
        ...
        self.genome_type_freq_history = []  # tracks per-gen genome type frequencies
        self.genome_seq_fitness = {}        # genome_key -> best record seen
        self.genome_all_records = []        # every individual every gen